...
| Correction targeting precision | Graphiti auto-invalidates the specific outdated edge | 🟡 Subject+object match invalidates all matches; subject-only corrections use cosine-similarity against a `minimumSimilarity` threshold to invalidate the *single* most-similar active fact, no-op if nothing clears the bar (`MemoryGraphStore.swift:104-120`). The no-op case is surfaced, not silent: `invalidateFacts` returns `Bool`, and `IngestionActor.enqueue` returns the list of corrections that matched nothing so a host app can tell the user "I'm not sure what to update" (`IngestionActorTests.swift` covers both the surfaced-failure and clean-success paths) | 🟡 Partial, deliberately conservative — but the failure mode is surfaced, not silent |
...
| Reranking | ✅ Gemini reranking | ✅ `Reranker` protocol + `ChatProviderReranker` (works with any `ChatProvider` — Gemini, OpenAI, Anthropic, local MLX), an opt-in async overload of `perTurnMemoryBlock`; off by default, falls back to unreranked hybrid-search order on any failure or malformed model output | ✅ Done (opt-in) |
| Automatic retrieval gating (skip GraphRAG when graph already fits in prompt) | ✅ `is_partial: false` short-circuits per-turn retrieval | ✅ `RetrievalService.isCompilationExhaustive()` — true only when the graph fit under `factLimit` at caching time AND the live active-fact count still matches (no drift since caching, since the compilation is cached per-session not per-turn); `perTurnMemoryBlock` gates on it, skipping the hybrid search entirely when true | ✅ Done |
...
| Reranking | ✅ Done, opt-in (`Reranker`/`ChatProviderReranker`) |
...
2. **Correction invalidation for subject-only facts is a similarity threshold, not true resolution — but the failure mode is already surfaced, not silent.** The current approach (invalidate the single most-similar active fact above `minimumSimilarity`, else no-op) is a reasonable, safety-first stopgap — it explicitly favors under-correcting over wrongly wiping unrelated facts. An earlier pass of this review flagged the no-op as silent and suggested surfacing it; on closer read, `invalidateFacts` already returns `Bool`, and `IngestionActor.enqueue` already returns the corrections that matched nothing (`failedCorrections`), logged via `os.Logger.notice` and covered by `IngestionActorTests` — a host app already has what it needs to tell the user "I'm not sure what to update." Nothing left to build here; the remaining gap is purely about whether `minimumSimilarity: 0.5` is well-calibrated in practice, which needs real usage data, not more code.
...
Degree-based entity ranking, automatic retrieval gating, and reranking (features 1 & 4) — the retrieval-quality features this doc tracked as genuinely lower-priority-but-buildable — are all implemented as of 2026-08-20/21: `EntityDegreeRanking` (ranks `sessionCompilation()`'s budget-constrained facts by subject-entity connection count before recency), `RetrievalService.isCompilationExhaustive()` (skips `perTurnMemoryBlock`'s hybrid search once the compilation provably covers every active fact, with an explicit drift check so a mid-session-ingested fact is never silently hidden from search), and `Reranker`/`ChatProviderReranker` (an opt-in async overload of `perTurnMemoryBlock` that re-scores hybrid-search results via any `ChatProvider`, off by default and fail-safe to the unreranked order on any malformed response — deliberately kept opt-in rather than wired into the default path, since it's the one addition here that reintroduces an LLM call into the retrieval hot path, cutting against this package's otherwise-deterministic, zero-LLM-overhead retrieval design).
...
**Overall assessment (2026-08-21):** the package has faithfully captured synapse-cortex's core cognitive-loop *ideas* (temporal graph, hybrid retrieval, correction-via-invalidation, LLM-driven extraction) in an architecture appropriate for its actual target (embedded, on-device, multi-provider), while consciously dropping the parts of synapse-cortex that are genuinely server/product features rather than memory-engine features. Every feature this doc tracked as a real, buildable gap across two rounds of re-verification and follow-up work is now implemented and tested: entity resolution has a deterministic fuzzy-match fix, memory CRUD primitives exist for a host app's browse/edit UI, Notion export/import + Gemini context caching + knowledge graph visualization were already built (an earlier pass just hadn't noticed), and degree-based ranking, automatic retrieval gating, and opt-in reranking closed out the retrieval-quality gaps. Extraction quality filtering was extended with embedding-similarity near-duplicate dedup and a tightened prompt after a practical-use complaint about low-value facts accumulating. What's left is calibration and judgment calls that need real usage data, not more code: whether `minimumSimilarity: 0.5` (correction targeting) and `duplicateSimilarityThreshold: 0.85` (near-dup dedup) are well-tuned in practice, and whether reranking's opt-in default is worth flipping for any host app once its actual retrieval-quality delta is observed.
