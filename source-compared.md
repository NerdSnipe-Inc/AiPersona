...
Reranking (feature 4) remains genuinely lower-priority — not because it's server-only, but because it matters more at graph/traffic scales this package isn't likely to reach on a personal device; worth revisiting if usage data says otherwise. Degree-based entity ranking and automatic retrieval gating (features 1 & 4) are no longer in that bucket — implemented 2026-08-20/21 via `EntityDegreeRanking` (ranks `sessionCompilation()`'s budget-constrained facts by subject-entity connection count before recency) and `RetrievalService.isCompilationExhaustive()` (skips `perTurnMemoryBlock`'s hybrid search once the compilation provably covers every active fact, with an explicit drift check so a mid-session-ingested fact is never silently hidden from search).
